...
let serverid = [
    '448188066524430337', // Test Server
    '555334013255155712', // Arizona Chandler
]
...
let mute = require('/system_files/mute').getmute(); // Команда mute
...
async function timer(){
    setInterval(async () => {
        ...
        let anonem = data_server.channels.find(c => c.name == 'объявление');
...
bot.on('message', async message => {
    ...
    if (message.type === "PINS_ADD") if (message.channel.name == "requests-for-roles" || message.channel.name == "объявление") message.delete();
    ...
    if (event.t == "MESSAGE_REACTION_ADD"){
        ...
        if (event_emoji_name == "🇩"){
            ...
            if (message.embeds[0].title == '`Discord » Проверка на валидность ник нейма.`'){
                ...
                field_channel.send(`<@${field_user.id}>, \`с вас сняли роль\`  <@&${field_role.id}>  \`по запросу от\` <@${field_author.id}> \`Одобрил:\` <@${member.id}>`);
                if (snyatie.has(field_author.id + `=>` + field_user.id)) snyatie.delete(field_author.id + `=>` + field_user.id);
                return message.delete();
            }
        }
